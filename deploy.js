...
const getCloudFormationOuputValue = (key) => {
    const command = `aws cloudformation describe-stacks --stack-name aws-s3-react-app --no-paginate --no-cli-pager --output text --query "Stacks[0].Outputs[?OutputKey=='${key}'].OutputValue"`;
//   const command = `aws cloudformation describe-stacks --stack-name aws-s3-react-app --no-paginate --no-cli-pager --output text --query "Stacks[0].Outputs[?OutputKey=='WebAppS3BucketName'].OutputValue"`;
  const output = child_process.execSync(command).toString();
